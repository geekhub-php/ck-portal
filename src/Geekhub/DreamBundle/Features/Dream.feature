...
Функционал: Страница мрии
  ...
  Сценарий: Создание мрии только авторизированым пользователем
    ...
    То выведи последний ответ сервера
    То я не должен видеть "Нова мрія"
    Тогда я вхожу как пользователь "Юля" с паролем "Юля"
    И я на странице "/"
    То я должен видеть "Вітаю, Юля Прудникова"
    Когда я кликаю по ссылке "Додати мрію"
    То я должен видеть "Нова мрія"

  Сценарий: Создание мечты
    Допустим я вхожу как пользователь "Юля" с паролем "Юля"
  ...
  Сценарий: Одобрение мечты
    Допустим админ одобрил мечту "Нужен новый ноутбук MacBook Air"
    И я на главной странице
    Когда я кликаю по ссылке "Нужен новый ноутбук MacBook Air"
    То я не должен видеть "Мрія знаходиться на модерації"

  Сценарий: Донейт мечты юзером
    Допустим я вхожу как пользователь "Алла" с паролем "Алла"
    И я на главной странице
    Когда я кликаю по ссылке "Нужен новый ноутбук MacBook Air"
    И я жду 5 секунд
    То я должен видеть "Підтримати мрію фінансово"
